...
describe('Integración DEV 1 (Usuarios) + DEV 2 (Etapas Productivas)', () => {
  ...
  it('Flujo completo: Registro de usuario -> Registro de Empresa -> Registro de EP', async () => {
    ...
    vi.spyOn(Document.prototype, 'save').mockImplementation(function() {
      return Promise.resolve(this);
    });
    ...
    vi.spyOn(ProductiveStage, 'findOne').mockImplementation((query) => {
      if (query && query.radicado) {
        return mockQuery;
      }
      return null;
    });
    
    vi.spyOn(ProductiveStage, 'create').mockImplementation(async (data) => {
      const doc = new ProductiveStage(data);
      doc._id = new mongoose.Types.ObjectId();
      return await doc.save();
    });

    vi.spyOn(Document, 'create').mockImplementation(async (data) => {
