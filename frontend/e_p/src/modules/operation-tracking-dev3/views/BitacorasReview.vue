    ...
    <div class="main-wrapper">
      ...
      <div class="content-columns">
        ...
        <aside class="column-fichas">
          ...
          <button 
            class="btn-new-bitacora mt-2" 
            style="background-color: #7c3aed !important; box-shadow: 0 2px 6px rgba(124, 58, 237, 0.2);"
            @click="openGeneralChatDrawer()"
          >
...
<!-- ═══ CAJÓN DE OBSERVACIONES FLOW CHAT GENERAL (RF-INS-30) ═══ -->
<Transition name="drawer">
  <div v-if="showGeneralChatDrawer" class="chat-drawer-backdrop" @click="closeGeneralChatDrawer">
    <div class="chat-drawer-content" @click.stop>
      <div class="chat-drawer-header">
        <div class="chat-drawer-title">
          ...
          <div>
            <h3>Chat del Flujo General</h3>
            <p>Observaciones y seguimiento para {{ apprenticeName }}</p>
          </div>
        </div>
        <button class="chat-close-btn" @click="closeGeneralChatDrawer">&times;</button>
      </div>

      <!-- Feed de mensajes -->
      <div class="chat-drawer-body">
        <div v-if="!stageDetails?.chatObservaciones || stageDetails.chatObservaciones.length === 0" class="chat-empty-state">
          <span class="material-symbols-outlined">chat_bubble_outline</span>
          <p>No hay mensajes en este flujo todavía.</p>
          <span class="empty-sub">Escriba una observación general a continuación para iniciar el diálogo.</span>
        </div>
        
        <div v-else class="chat-feed-list">
          <div 
            v-for="(msg, idx) in stageDetails.chatObservaciones" 
            :key="idx" 
      ...
      <div class="chat-drawer-footer">
        <textarea 
          v-model="generalChatMessageText" 
          placeholder="Escriba una observación o comentario..." 
          class="chat-input"
          rows="2"
          @keydown.enter.prevent="sendGeneralChatMessage"
        ></textarea>
        <button 
          class="btn-send-message" 
          @click="sendGeneralChatMessage" 
          :disabled="isSendingGeneralChatMessage || !generalChatMessageText.trim()"
        >
          <span class="material-symbols-outlined" :class="{ 'animate-spin': isSendingGeneralChatMessage }">
            {{ isSendingGeneralChatMessage ? 'sync' : 'send' }}
          </span>
        </button>
      </div>
    </div>
...
// --- GENERAL FLOW OBSERVATIONS CHAT DRAWER (RF-INS-30) ---
import http from '../../../core/api/http'

const showGeneralChatDrawer = ref(false)
const generalChatMessageText = ref('')
const isSendingGeneralChatMessage = ref(false)

const openGeneralChatDrawer = () => {
  showGeneralChatDrawer.value = true
}

const closeGeneralChatDrawer = () => {
  showGeneralChatDrawer.value = false
}
...
const sendGeneralChatMessage = async () => {
  ...
  try {
    ...
    // Recargar detalles de la etapa para ver el mensaje
